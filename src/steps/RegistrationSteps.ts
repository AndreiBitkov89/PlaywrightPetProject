...
export class RegistrationSteps {
    ...
    async fillAllRequiredFields(
        ...
        await test.step("Fill all required fields for registration", async () => {
            ...
            await this.ctx.registrationPage.fillPasswordForConfirm(wrongPassword ?? user.password);
        })
    }
    ...
    async fillFields(
        ...
        await test.step("Fill all required fields", async () => {
            await this.fillAllRequiredFields(
                user,
                wrongPassword,
            );
        });
    ...
    async assertErrorText(
        ...
        const errorLocators: Record<ErrorFieldRegistration, Locator> = {
            ...
            [ErrorFieldRegistration.FirstName]: this.ctx.registrationPage.getErrorName,
            [ErrorFieldRegistration.Surname]: this.ctx.registrationPage.getErrorSurname,
            [ErrorFieldRegistration.Password]: this.ctx.registrationPage.getErrorPassword,
            [ErrorFieldRegistration.ConfirmPassword]: this.ctx.registrationPage.getErrorConfirmPassword,
        };
        const selectorText = await errorLocators[errorField].textContent();
        expect(selectorText).toEqual(
            errorText,
        );
    }
}
